	...
	SubShader {
		...
		Pass {  
			CGPROGRAM
				...
				float4 place(float f) {
		            float t = max(0., min(.999, f)) * (_UNPnts-1);
		            int n = t;//int(t);
		            float4 uData1 = tex2Dlod (_PosTex, float4(f,0,0,0));
...
//		            float4 uData2 = tex2D(_PosTex, float2(f+.0001,0.));
		            return float4(lerp(uData1,uData2, t-float(n) ).xyz,1.);
		         }
...
//					o.vertex = mul(UNITY_MATRIX_MVP, v.vertex);
	            	...
	            	float4 p0 = mul(UNITY_MATRIX_MVP, place(position.x ));
	            	float4 p1 = mul(UNITY_MATRIX_MVP, place(position.x + (1/_UNPnts)));
	            	o.vertex = p1 + normalize(float4(p1.y-p0.y, p0.x-p1.x, 0., 0.)) * position.y * _LineWidth;// tex2Dlod (_PosTex, float4(uv.x,uv.y,0,0)).a * _LineWidth;
...
//					fixed4 col2 = tex2D(_SpriteTex, i.texcoord2);
					...
					return wave*sprite*hue*sprite.a;
				}
			ENDCG
		}
	}
